...
localparam US_MSB = US_DW == 32 ? 1 : US_DW == 64 ? 2 : US_DW == 128 ? 3 : 4;
localparam DS_MSB = DS_DW == 32 ? 1 : DS_DW == 64 ? 2 : DS_DW == 128 ? 3 : DS_DW == 256 ? 4 : 5;
localparam US_IDX = US_MSB + 1;
localparam US_SIZE = (US_DW == 32) ? 3'd2 : (US_DW == 64) ? 3'd3 : (US_DW == 128) ? 3'd4 : 3'd5;
localparam DS_SIZE = (DS_DW == 64) ? 3'd3 : (DS_DW == 128) ? 3'd4 : (DS_DW == 256) ? 3'd5 : 3'd6;
localparam US_MASK = (US_DW == 256) ? 5'h7 : (US_DW == 128) ? 5'h3 : (US_DW == 64) ? 5'h1 : 5'h0;
localparam US_BEAT_SIZE = US_DW == 32 ? 5'd1 : US_DW == 64 ? 5'd2 : US_DW == 128 ? 5'd4 : 5'd8;
...
wire nds_unused_a_offset_add_one_co;
...
generate
    if (RAR_SUPPORT) begin:gen_ds_a_reset
        always @(posedge clk or negedge resetn) begin
            if (!resetn) begin
                ds_a_opcode <= 3'b0;
                ds_a_param <= 3'b0;
                ds_a_user <= {A_UW{1'b0}};
                ds_a_size <= 3'b0;
                ds_a_address <= {AW{1'b0}};
                ds_a_source <= {OW{1'b0}};
                ds_a_corrupt <= 1'b0;
            end
            else if (ds_a_ctrl_en) begin
...
generate
    ...
    for (i = 0; i < RATIO; i = i + 1) begin:gen_a
        ...
        if (RAR_SUPPORT) begin:gen_ds_a_data_mask_reset
            always @(posedge clk or negedge resetn) begin
                if (!resetn) begin
                    ds_a_data[i * US_DW +:US_DW] <= {US_DW{1'b0}};
                end
                else if (ds_a_data_en[i]) begin
            ...
            always @(posedge clk or negedge resetn) begin
                if (!resetn) begin
                    ds_a_mask[i * (US_DW / 8) +:(US_DW / 8)] <= {(US_DW / 8){1'b0}};
                end
                else if (ds_a_mask_en[i]) begin
                    ds_a_mask[i * (US_DW / 8) +:(US_DW / 8)] <= a_mask_nx;
                end
            end

        end
        else begin:gen_ds_a_data_mask
            always @(posedge clk) begin
...
assign {nds_unused_a_offset_add_one_co,a_offset_add_one} = a_offset[6:2] + US_BEAT_SIZE;
...
generate
    if ((US_DW == 32) & (DS_DW == 64)) begin:a_last_32_64
        assign a_last_lane = (a_size <= 3'd2) | ((a_size >= 3'd3) & (&a_offset[2]));
    end
    else if ((US_DW == 32) & (DS_DW == 128)) begin:a_last_32_128
        assign a_last_lane = (a_size <= 3'd2) | ((a_size == 3'd3) & (&a_offset[2])) | ((a_size >= 3'd4) & (&a_offset[3:2]));
    end
    else if ((US_DW == 32) & (DS_DW == 256)) begin:a_last_32_256
        assign a_last_lane = (a_size <= 3'd2) | ((a_size == 3'd3) & (&a_offset[2])) | ((a_size == 3'd4) & (&a_offset[3:2])) | ((a_size >= 3'd5) & (&a_offset[4:2]));
    end
    else if ((US_DW == 32) & (DS_DW == 512)) begin:a_last_32_512
        assign a_last_lane = (a_size <= 3'd2) | ((a_size == 3'd3) & (&a_offset[2])) | ((a_size == 3'd4) & (&a_offset[3:2])) | ((a_size == 3'd5) & (&a_offset[4:2])) | ((a_size >= 3'd6) & (&a_offset[5:2]));
    end
    else if ((US_DW == 64) & (DS_DW == 128)) begin:a_last_64_128
        assign a_last_lane = (a_size <= 3'd3) | ((a_size >= 3'd4) & (&a_offset[3]));
    end
    else if ((US_DW == 64) & (DS_DW == 256)) begin:a_last_64_256
        assign a_last_lane = (a_size <= 3'd3) | ((a_size == 3'd4) & (&a_offset[3])) | ((a_size >= 3'd5) & (&a_offset[4:3]));
    end
    else if ((US_DW == 64) & (DS_DW == 512)) begin:a_last_64_512
        assign a_last_lane = (a_size <= 3'd3) | ((a_size == 3'd4) & (&a_offset[3])) | ((a_size == 3'd5) & (&a_offset[4:3])) | ((a_size >= 3'd6) & (&a_offset[5:3]));
    end
    else if ((US_DW == 128) & (DS_DW == 256)) begin:a_last_128_256
        assign a_last_lane = (a_size <= 3'd4) | ((a_size >= 3'd5) & (&a_offset[4]));
    end
    else if ((US_DW == 128) & (DS_DW == 512)) begin:a_last_128_512
        assign a_last_lane = (a_size <= 3'd4) | ((a_size == 3'd5) & (&a_offset[4])) | ((a_size >= 3'd6) & (&a_offset[5:4]));
    end
    else if ((US_DW == 256) & (DS_DW == 512)) begin:a_last_256_512
        assign a_last_lane = (a_size <= 3'd5) | ((a_size >= 3'd6) & (&a_offset[5]));
    end
endgenerate
generate
    ...
    else if ((US_DW == 32) & (DS_DW == 512)) begin:gen_32_512
        assign a_lane_sel_get = ({16{(a_size == 3'd3) && a_lane_onehot[0]}} & 16'h0003) | ({16{(a_size == 3'd3) && a_lane_onehot[2]}} & 16'h000c) | ({16{(a_size == 3'd3) && a_lane_onehot[4]}} & 16'h0030) | ({16{(a_size == 3'd3) && a_lane_onehot[6]}} & 16'h00c0) | ({16{(a_size == 3'd3) && a_lane_onehot[8]}} & 16'h0300) | ({16{(a_size == 3'd3) && a_lane_onehot[10]}} & 16'h0c00) | ({16{(a_size == 3'd3) && a_lane_onehot[12]}} & 16'h3000) | ({16{(a_size == 3'd3) && a_lane_onehot[14]}} & 16'hc000) | ({16{(a_size == 3'd4) && a_lane_onehot[0]}} & 16'h000f) | ({16{(a_size == 3'd4) && a_lane_onehot[4]}} & 16'h00f0) | ({16{(a_size == 3'd4) && a_lane_onehot[8]}} & 16'h0f00) | ({16{(a_size == 3'd4) && a_lane_onehot[12]}} & 16'hf000) | ({16{(a_size == 3'd5) && a_lane_onehot[0]}} & 16'h00ff) | ({16{(a_size == 3'd5) && a_lane_onehot[8]}} & 16'hff00) | ({16{(a_size >= 3'd6)}} & 16'hffff);
    end
    ...
    else if ((US_DW == 64) & (DS_DW == 512)) begin:gen_64_512
        assign a_lane_sel_get = ({8{(a_size == 3'd4) && a_lane_onehot[0]}} & 8'h03) | ({8{(a_size == 3'd4) && a_lane_onehot[2]}} & 8'h0c) | ({8{(a_size == 3'd4) && a_lane_onehot[4]}} & 8'h30) | ({8{(a_size == 3'd4) && a_lane_onehot[6]}} & 8'hc0) | ({8{(a_size == 3'd5) && a_lane_onehot[0]}} & 8'h0f) | ({8{(a_size == 3'd5) && a_lane_onehot[4]}} & 8'hf0) | ({8{(a_size >= 3'd6)}} & 8'hff);
    end
    else if ((US_DW == 128) & (DS_DW == 256)) begin:gen_128_256
        assign a_lane_sel_get = ({2{(a_size >= 3'd5)}} & 2'h3);
    end
    else if ((US_DW == 128) & (DS_DW == 512)) begin:gen_128_512
        assign a_lane_sel_get = ({4{(a_size == 3'd5) && a_lane_onehot[0]}} & 4'h3) | ({4{(a_size == 3'd5) && a_lane_onehot[2]}} & 4'hc) | ({4{(a_size >= 3'd6)}} & 4'hf);
    end
    else if ((US_DW == 256) & (DS_DW == 512)) begin:gen_256_512
        assign a_lane_sel_get = ({2{(a_size >= 3'd6)}} & 2'h3);
    end
endgenerate
...
generate
    ...
    for (j = 0; j < OST_GET_NUM; j = j + 1) begin:gen_ent
        ...
        wire [6:2] offset_nx;
        wire offset_en;
        wire [6:2] offset_add_one;
        wire nds_unused_offset_add_one;
        wire [6:2] offset_masked;
        wire us_d_grant_hit = us_d_grant & us_d_hit;
        assign {nds_unused_offset_add_one,offset_add_one} = offset + US_BEAT_SIZE;
        always @(posedge clk or negedge resetn) begin
            if (!resetn) begin
                valid <= 1'b0;
            end
        ...
        assign valid_clr = us_d_grant_hit & d_opcode_accessackdata & last;
        assign valid_nx = valid_set | (valid & ~valid_clr);
        assign offset_en = valid_set | (us_d_grant_hit & d_opcode_accessackdata);
        ...
        assign last_lane = (size <= US_SIZE) | ((US_DW == 32) & (DS_DW == 64) & (size >= 3'd3) & (&offset_masked[2:2])) | ((US_DW == 32) & (DS_DW == 128) & (size == 3'd3) & (&offset_masked[2:2])) | ((US_DW == 32) & (DS_DW == 128) & (size >= 3'd4) & (&offset_masked[3:2])) | ((US_DW == 32) & (DS_DW == 256) & (size == 3'd3) & (&offset_masked[2:2])) | ((US_DW == 32) & (DS_DW == 256) & (size == 3'd4) & (&offset_masked[3:2])) | ((US_DW == 32) & (DS_DW == 256) & (size >= 3'd5) & (&offset_masked[4:2])) | ((US_DW == 32) & (DS_DW == 512) & (size == 3'd3) & (&offset_masked[2:2])) | ((US_DW == 32) & (DS_DW == 512) & (size == 3'd4) & (&offset_masked[3:2])) | ((US_DW == 32) & (DS_DW == 512) & (size == 3'd5) & (&offset_masked[4:2])) | ((US_DW == 32) & (DS_DW == 512) & (size >= 3'd6) & (&offset_masked[5:2])) | ((US_DW == 64) & (DS_DW == 128) & (size >= 3'd4) & (&offset_masked[3:3])) | ((US_DW == 64) & (DS_DW == 256) & (size == 3'd4) & (&offset_masked[3:3])) | ((US_DW == 64) & (DS_DW == 256) & (size >= 3'd5) & (&offset_masked[4:3])) | ((US_DW == 64) & (DS_DW == 512) & (size == 3'd4) & (&offset_masked[3:3])) | ((US_DW == 64) & (DS_DW == 512) & (size == 3'd5) & (&offset_masked[4:3])) | ((US_DW == 64) & (DS_DW == 512) & (size >= 3'd6) & (&offset_masked[5:3])) | ((US_DW == 128) & (DS_DW == 256) & (size >= 3'd5) & (&offset_masked[4:4])) | ((US_DW == 128) & (DS_DW == 512) & (size == 3'd5) & (&offset_masked[4:4])) | ((US_DW == 128) & (DS_DW == 512) & (size >= 3'd6) & (&offset_masked[5:4])) | ((US_DW == 256) & (DS_DW == 512) & (size >= 3'd6) & (&offset_masked[5:5]));
